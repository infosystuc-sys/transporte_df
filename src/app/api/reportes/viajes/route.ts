...
export async function GET(request: NextRequest) {
  ...
  let consulta = db
    .select({
      ...
      titular_nombre: viajes.titular_nombre,
      titular_cuit: viajes.titular_cuit,
      destinatario_nombre: viajes.destinatario_nombre,
      destinatario_cuit: viajes.destinatario_cuit,
  ...
  const buffer = generarXlsx([
    {
      ...
      filas: filas.map((f) => ({
        ...
        CTG: f.ctg,
        "N° CPE": f.cpe_nro,
        "Cliente (flete pagador)": f.cliente,
        Titular: f.titular_nombre,
        "CUIT titular": f.titular_cuit,
        Destinatario: f.destinatario_nombre,
        "CUIT destinatario": f.destinatario_cuit,
        Chofer: f.chofer,
